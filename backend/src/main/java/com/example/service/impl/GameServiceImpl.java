...
import com.example.data.model.dao.GameChatMessage;
...
import com.example.data.model.enums.DefaultWord;
...
public class GameServiceImpl implements GameService {
    ...
    public GameSession createGame(User user) {
        ...
        do {
            ...
            gameSession.getReadyStatus().put(user.getId().toString(), false);
    ...
    @Override
    public synchronized GameSession joinGame(String roomId, User user) {
    ...
    public synchronized void toggleReady(User user, String roomId) {
        ...
        session.getReadyStatus().put(user.getId().toString(), newStatus);
    ...
    public synchronized void submitWord(User user, String word, String roomId) {
        ...
        if (gameSession != null && gameSession.getPhase() == GamePhase.WORD_SUBMISSION) {
            gameSession.getSubmittedWords().put(user.getId().toString(), word);
    ...
    public void processGuess(User user, String guess, String roomId, String sessionId) {
        ...
        String wordAuthorId = gameSession.getSubmittedWords().entrySet().stream()
        ...
        if (user.getId().equals(currentDrawingUser.getId()) || user.getId().equals(wordAuthorId)) {
            sendPrivateMessage(user.getId(), "System: You can't guess in this round!");
            return;
        }

        GameChatMessage chatMessage = new GameChatMessage(user.getUsername(), guess);
        ...
        switch (gameSession.getPhase()) {
            case WORD_SUBMISSION:
                gameSession.getUsers().forEach(user ->
                        gameSession.getSubmittedWords().computeIfAbsent(user.getId().toString(),
    ...
    private void handleCorrectGuess(GameSession gameSession, User guesser) {
        ...
        GameChatMessage systemMsg = new GameChatMessage("System",
    ...
    private void sendPrivateMessage(UUID userId, String message) {
        ...
        messagingTemplate.convertAndSendToUser(
                userIdString,
                "/queue/notifications",
                new GameChatMessage("System ", message)
        );
    }
}
